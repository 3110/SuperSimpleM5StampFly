...
#include <iostream>
static constexpr const uint8_t log_colors_serial[] = {
    98, 91, 93, 92, 96, 97,
};
#else
static constexpr const uint8_t log_colors_serial[] = {
...
#endif
static constexpr const uint8_t log_colors_display[] = {
    0xFF, 0xE0, 0xFC, 0x18, 0x1F, 0x92,
};
...
void Log_Class::update_level(void) {
    _level_maximum = std::max(
        std::max(_log_level[log_target_serial], _log_level[log_target_display]),
        _log_level[log_target_callback]);
}

void Log_Class::output(esp_log_level_t level, bool suffix,
    ...
    if (_log_level[log_target_serial] >= level) {
        const char* suf = (suffix && _suffix[log_target_serial])
                              ? _suffix[log_target_serial]
                              : "";

        if (level != ESP_LOG_NONE && _use_color[log_target_serial]) {
            ::printf("\033[0;%dm%s\033[0m%s", log_colors_serial[level], str,
                     suf);
        } else {
            ::printf("%s%s", str, suf);
        }
    ...
    if (_display && _log_level[log_target_display] >= level) {
        if (level != ESP_LOG_NONE && _use_color[log_target_display]) {
            ...
            if (style.fore_rgb888 == style.back_rgb888) {
                ...
                _display->setTextColor(
                    log_colors_display[level],
                    m5gfx::color_convert<m5gfx::rgb332_t, m5gfx::rgb888_t>(
                        style.back_rgb888));
            }
            _display->print(str);
        ...
        if (suffix && _suffix[log_target_display]) {
            _display->print(_suffix[log_target_display]);
        }
    }
#endif
    ...
    if (_log_level[log_target_callback] >= level && _callback != nullptr) {
        ...
        if (suffix && _suffix[log_target_callback]) {
            _callback(level, _log_level[log_target_callback],
                      _suffix[log_target_callback]);
        }
    }
}
...
void Log_Class::dump(const void* a, uint32_t len, esp_log_level_t level) {
    ...
    do {
        int pos =
            snprintf(buf, sizeof(buf), "0x%08" PRIxPTR "|", (uintptr_t)addr);
        // printf("0x%08x|", (uintptr_t)addr);
        int l = len > 4 ? 4 : len;
        for (int i = 0; i < l; ++i) {
            unsigned int tmp = addr[i];
            pos += snprintf(&buf[pos], (int)sizeof(buf) - pos,
                            " %02x %02x %02x %02x ", tmp & 0xFF,
                            (tmp >> 8) & 0xFF, (tmp >> 16) & 0xFF, (tmp >> 24));
        }
        for (int i = l; i < 4; ++i) {
