...
#define M5UNIFIED_LOG_FORMAT(letter, format)                    \
    "[%6u][" #letter "][%s:%u] %s(): " format, m5gfx::millis(), \
        m5::Log_Class::pathToFileName(__FILE__), __LINE__, __FUNCTION__
#endif
...
public:
    ...
    esp_log_level_t getLogLevel(log_target_t target) const {
        return _log_level[target];
    }

    /// Set the text to be added to the end of the log.
    void setSuffix(log_target_t target, const char* suffix) {
        if (target < log_target_max) {
            _suffix[target] = suffix;
        }
    }

    /// set logging callback function / functor .
    /// @param function Pointer to a user-defined function that takes three
    /// arguments: esp_log_level_t , bool, const char*.
    void setCallback(std::function<void(esp_log_level_t log_level,
                                        bool use_color, const char* log_text)>
                         function) {
        _callback = function;
    };
    ...
    void dump(const void* addr, uint32_t len,
              esp_log_level_t level = esp_log_level_t::ESP_LOG_NONE);
...
private:
    ...
    void output(esp_log_level_t level, bool suffix,
                const char* __restrict format, va_list arg);
    void update_level(void);

    std::function<void(esp_log_level_t log_level, bool use_color,
                       const char* log_text)>
        _callback;
...
#endif
    esp_log_level_t _log_level[log_target_max] = {
        _level_maximum, _level_maximum, _level_maximum};
